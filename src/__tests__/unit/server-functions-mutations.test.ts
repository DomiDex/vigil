...
describe("server functions -- mutations", () => {
  ...
  describe("triggerDream", () => {
    it("calls handleDreamTrigger with context and repo", async () => {
      ...
      const handlerSpy = spyOn(dreamsMod, "handleDreamTrigger").mockResolvedValue(
        "<div>ok</div>",
      );

      const { triggerDream } = await import(
  ...
  describe("createTask", () => {
    it("calls handleTaskCreate with context and FormData", async () => {
      ...
      const handlerSpy = spyOn(tasksMod, "handleTaskCreate").mockReturnValue(
        "<div>created</div>",
      );
  ...
  describe("activateTask", () => {
    it("calls handleTaskActivate with context and id", async () => {
      const tasksMod = await import("../../dashboard/api/tasks.ts");
      const handlerSpy = spyOn(tasksMod, "handleTaskActivate").mockReturnValue(
        "<div>activated</div>",
      );

      const { activateTask } = await import(
        "../../../dashboard-v2/src/server/functions.ts"
      );
  ...
  describe("completeTask", () => {
    it("calls handleTaskComplete with context and id", async () => {
      const tasksMod = await import("../../dashboard/api/tasks.ts");
      const handlerSpy = spyOn(tasksMod, "handleTaskComplete").mockReturnValue(
        "<div>completed</div>",
      );
  ...
  describe("failTask", () => {
    it("calls handleTaskFail with context and id", async () => {
      const tasksMod = await import("../../dashboard/api/tasks.ts");
      const handlerSpy = spyOn(tasksMod, "handleTaskFail").mockReturnValue(
        "<div>failed</div>",
      );

      const { failTask } = await import(
        "../../../dashboard-v2/src/server/functions.ts"
      );
  ...
  describe("updateTask", () => {
    it("calls handleTaskUpdate with context, id, and FormData", async () => {
      const tasksMod = await import("../../dashboard/api/tasks.ts");
      const handlerSpy = spyOn(tasksMod, "handleTaskUpdate").mockReturnValue(
        "<div>updated</div>",
      );
  ...
  describe("cancelTask", () => {
    it("calls handleTaskCancel with context and id", async () => {
      const tasksMod = await import("../../dashboard/api/tasks.ts");
      const handlerSpy = spyOn(tasksMod, "handleTaskCancel").mockReturnValue(
        "<div>cancelled</div>",
      );
  ...
  describe("approveAction", () => {
    it("calls handleApprove with context and id", async () => {
      ...
      const handlerSpy = spyOn(actionsMod, "handleApprove").mockResolvedValue(
        "<div>approved</div>",
      );
  ...
  describe("rejectAction", () => {
    it("calls handleReject with context and id", async () => {
      const actionsMod = await import("../../dashboard/api/actions.ts");
      const handlerSpy = spyOn(actionsMod, "handleReject").mockReturnValue(
        "<div>rejected</div>",
      );

      const { rejectAction } = await import(
        "../../../dashboard-v2/src/server/functions.ts"
      );
  ...
  describe("askVigil", () => {
    it("calls handleAsk with context, question, and optional repo", async () => {
      ...
      const handlerSpy = spyOn(memoryMod, "handleAsk").mockResolvedValue(
        "<div>The answer is 42</div>",
      );

      const { askVigil } = await import(
        "../../../dashboard-v2/src/server/functions.ts"
      );
  ...
  describe("createSchedule", () => {
    it("calls handleSchedulerCreate with context and FormData", async () => {
      ...
      const handlerSpy = spyOn(schedulerMod, "handleSchedulerCreate").mockResolvedValue(
        "<div>created</div>",
      );
  ...
  describe("deleteSchedule", () => {
    it("calls handleSchedulerDelete with context and id", async () => {
      const schedulerMod = await import("../../dashboard/api/scheduler.ts");
      const handlerSpy = spyOn(schedulerMod, "handleSchedulerDelete").mockReturnValue(
        "<div>deleted</div>",
      );
  ...
  describe("triggerSchedule", () => {
    it("calls handleSchedulerTrigger with context and id", async () => {
      const schedulerMod = await import("../../dashboard/api/scheduler.ts");
      const handlerSpy = spyOn(schedulerMod, "handleSchedulerTrigger").mockResolvedValue(
        "<div>triggered</div>",
      );

      const { triggerSchedule } = await import(
        "../../../dashboard-v2/src/server/functions.ts"
      );
